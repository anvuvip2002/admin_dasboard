...
const EditCinemas = () => {
  ...
  const defaultValue = () => {
    ...
    for (var i = 0; i < 63; i++) {
      ...
      if (option.value == cinema.province.id)
        {options[i].defaultSelected = i;
        setProvince(cinema.province)}
    }
  ...
  const handleProvinceChange = (event) => {
    ...
    try {
      axios.get("http://20.214.254.141:3000/province/" + provinceId)
        .then(response => {
          const selectedProvince = response.data;
  ...
  const handleSubmit = () => {
    ...
    axios.put("http://20.214.254.141:3000/cinema/"+id, newCinema).then(response => {
        ...
        <form>
          <table>
            <tr>
              ...
              <td><input type="text" id="name" name="name" placeholder={cinema.name} onChange={(event) => setName(event.target.value)} required/></td>
            </tr>
            <tr>
              <td><label htmlFor="address">Address:</label></td>
              <td><input type="text" id="address" name="address" placeholder={cinema.address} onChange={(event) => setAddress(event.target.value)} required /></td>
            </tr>
            <tr>
              <td><label htmlFor="address_url">Address URL:</label></td>
              <td><input type="url" id="address_url" name="address_url" placeholder={cinema.address_url} onChange={(event) => setAddressUrl(event.target.value)} required /></td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td><input type="number" id="number_of_rooms" name="number_of_rooms" placeholder={cinema.number_of_rooms} onChange={(event) => setNumRooms(event.target.value)} required/></td>
            </tr>
            <tr>
              <button className="submitButton" onClick={() => handleSubmit()}>Save Changes</button>
            </tr>
          </table>
        </form>
      </div>
